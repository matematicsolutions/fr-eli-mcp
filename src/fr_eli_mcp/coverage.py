...
SOURCE = 'legifrance.gouv.fr via the PISTE API'
...
_FAMILIES: list[dict] = [{'name': 'Legislation (LODA, codes)', 'tool': 'fr_search / fr_get_act / fr_get_text'}, {'name': 'Case law (JURI)', 'tool': 'fr_get_decision'}, {'name': 'Agreements and deliberations', 'tool': 'fr_get_convention / fr_get_deliberation / fr_get_company_agreement'}]

_GAPS: list[dict] = [{'id': 'FR-001', 'family': 'Legislation (LODA, codes)', 'missing': 'The PISTE consult API returns a null ELI for legislation, so eli_uri carries the stable Legifrance resource URL instead of a native ELI identifier.', 'fallback': 'Treat eli_uri as a Legifrance CID-keyed URL, not an /eli/ path.'}, {'id': 'FR-002', 'family': 'Legislation (LODA, codes)', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
