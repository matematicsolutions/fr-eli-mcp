...
from fr_eli_mcp.coverage import build_coverage
from fr_eli_mcp.server import fr_coverage


def _coverage():
    fn = getattr(fr_coverage, "fn", fr_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FR_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("fr_coverage" in line for line in lines), (
        "an audit entry was written but it does not name fr_coverage"
    )
